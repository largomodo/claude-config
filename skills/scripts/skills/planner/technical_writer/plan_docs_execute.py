    ...
    if step == 1:
        ...
        return {
            ...
            "actions": [
                ...
                "Carry each decision's rationale INTO the comment text itself,",
                "rewritten as self-contained timeless prose. Do NOT cite DL-XXX",
                "ids in doc_diff content -- the decision log is a planning",
                "artifact that does not ship with the code, so such refs dangle",
                "for readers and fail the execution-phase temporal QR.",
            ],
        ...
        return {
            ...
            "actions": [
                ...
                "     - Each decision's rationale should be EXPRESSED in at least",
                "       one doc_diff, rewritten as self-contained timeless prose",
                "     - NEVER write plan ids into doc_diff content: no (ref: DL-XXX),",
                "       DL-XXX, CI-/CC-/M- ids, plan file names, or dates -- the plan",
                "       does not ship with the code, so those refs are baseline",
                "       references and fail the execution-phase temporal QR",
                "     - Use DL ids only in your working notes below and in",
                "       decision_refs metadata, never in doc_diff text",
                "",
        ...
        return {
            ...
            "actions": [
                ...
                "  - BAD: plan-artifact references: '(ref: DL-XXX)', milestone or",
                "    change ids (M-001, CC-M-001-001, CI-M-001-001), plan file",
                "    names, dates, 'per the plan', 'per this milestone'",
                "  - GOOD: Timeless present tense describing what IS, carrying the",
                "    decision's WHY in the text itself (self-contained -- no",
                "    pointer to the decision log, which does not ship)",
            ],
        ...
        return {
            ...
            "actions": [
                ...
                "  [ ] No plan-artifact refs in doc_diff additions -- mechanical scan:",
                "      cat $STATE_DIR/plan.json | jq -r '.milestones[].code_changes[].doc_diff' \\",
                "        | grep -nE '\\(ref:|DL-[0-9]|CC-M-|CI-M-'   # expect no output",
                "  [ ] Each decision's rationale is expressed in some doc_diff as",
                "      self-contained timeless prose (semantic check, not id grep)",
                "",
